...
const loadDataAsync = (data => (dataType = types.FlagType.EMOJI) => {
  return new Promise((resolve, reject) => {
    switch (dataType) {
      case types.FlagType.FLAT:
...
const getImageFlagAsync = async (countryCode = 'FR') => {
  const countries = await loadDataAsync(types.FlagType.FLAT);
  if (!countries) {
...
const getCountriesAsync = async (flagType, translation = 'common', region, subregion, countryCodes, excludeCountries, preferredCountries, withAlphaFilter) => {
  ...
  if (preferredCountries && !withAlphaFilter) {
    const newCountryCodeList = [...preferredCountries, ...types.CountryCodeList.filter(code => !preferredCountries.includes(code))];
    ...
    const countries = types.CountryCodeList.filter(isCountryPresent(countriesRaw)).map(cca2 => ({
      ...{
        ...countriesRaw[cca2],
        name: countriesRaw[cca2].name[translation] || countriesRaw[cca2].name['common']
      },
